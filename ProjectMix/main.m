...
// http://www.imyuyang.com/2018/02/15/iOS马甲包混淆方案/
...
//        NSString *methodName = [NSString stringWithFormat:@"- (void)%@:(NSString *)%@;\n", apiName,paramName];
        NSString *argStr = [NSString stringWithFormat:@"- (void)%@:(NSString *)", apiName];
        NSString *str = [NSString stringWithFormat:@"- (void)%@;", apiName];
        if([fileContent containsString:argStr] || [fileContent containsString:str]){
            NSLog(@"methodName 存在 apiName -- %@",apiName);
...
//        NSString *methodName = [NSString stringWithFormat:@"- (void)%@:(NSString *)%@ {\n", apiName, paramName];
         NSString *argStr = [NSString stringWithFormat:@"- (void)%@:(NSString *)", apiName];
        NSString *str = [NSString stringWithFormat:@"- (void)%@ {", apiName];
        if([mfileContent containsString:argStr] ||[mfileContent containsString:str]){
            return;
        }
        if(kSpamCount%4 == 0){
            NSString *s = @"@end";
